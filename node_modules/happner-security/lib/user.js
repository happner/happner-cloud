...
UserManager.prototype.create = function($happn, params, callback) {
	...
	var updateEventData = {channel:_const.CHANNEL.USER, 
						   action:_const.ACTION.CREATE,
						   data:user};

	console.log('in backend u update, doing emit', updateEventData);					   

	$happn.exchange.happnerSecurityNetwork.securityEvent(updateEventData, function(e){
...
UserManager.prototype.update = function($happn, params, callback) {
  console.log('in backend u update', params);

	try{

		if (!_users[params.data.name]) throw new Error('user does not exist');

		var user = _users[params.data.name];
		user.description = params.data.description;
		user.groups = params.data.groups;
		
		var updateEventData = {channel:_const.CHANNEL.USER, 
							   action:_const.ACTION.UPDATE,
							   data:user};

			console.log('in backend g update, doing emit', updateEventData);					   

		$happn.exchange.happnerSecurityNetwork.securityEvent(updateEventData, callback);
...
UserManager.prototype.start = function($happn, params, callback){
	...
	try{
		...
		$happn.event.happnerSecurityNetwork.on(_const.CHANNEL.NETWORK, function(data){
